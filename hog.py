...
def swine_align(player_score, opponent_score):
    ...
    if min==0:
        return False
    while (player_score%min or opponent_score%min) and min>=10:
        min=min-1
    if min>=10:
        return True
    else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    if who==0:
        def announce_highest_who(score0,score1,last_score=last_score,running_high=running_high):
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while k<=10:
        sum=0
        sum=make_averaged(roll_dice,trials_count)(k,dice)*trials_count
        if sum>max_sum:
            max_sum=sum
            num=k
        k+=1
