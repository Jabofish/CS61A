...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) >0,"an interval that spans zero is used as a divisor"
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x)<=-b/(2*a) and upper_bound(x)>=-b/(2*a):
        return interval(min(f(lower_bound(x)),f(-b/(2*a)),f(upper_bound(x))),max(f(lower_bound(x)),f(-b/(2*a)),f(upper_bound(x))))
    else:
